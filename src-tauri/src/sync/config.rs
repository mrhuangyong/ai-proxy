...
pub async fn load_config(pool: &SqlitePool) -> SyncResult<SyncConfig> {
    ...
    Ok(SyncConfig {
        ...
        webdav_path: {
            ...
            if p.is_empty() { "ai-proxy-backups/".into() } else { p }
        },
...
pub async fn save_config(pool: &SqlitePool, cfg: &SyncConfig) -> SyncResult<()> {
    let mut updates: Vec<(&str, String)> = vec![
        ("sync_enabled", if cfg.enabled { "true".into() } else { "false".into() }),
        ("sync_webdav_url", cfg.webdav_url.clone()),
        ("sync_webdav_username", cfg.webdav_username.clone()),
        ("sync_webdav_path", cfg.webdav_path.clone()),
        ("sync_auto_enabled", if cfg.auto_enabled { "true".into() } else { "false".into() }),
        ("sync_auto_interval_minutes", cfg.auto_interval_minutes.to_string()),
        ("sync_on_change", if cfg.sync_on_change { "true".into() } else { "false".into() }),
    ];
    // Only update password if a new one is provided (non-empty).
    if !cfg.webdav_password.is_empty() {
        let (ct, nonce) = encrypt_api_key(&cfg.webdav_password)
...
pub async fn update_sync_status(pool: &SqlitePool, status: &str, error: &str) -> SyncResult<()> {
    ...
    sqlx::query("UPDATE settings SET value = ?, updated_at = datetime('now') WHERE key = 'sync_last_error'")
...
fn decrypt_password(stored: &str) -> SyncResult<String> {
    ...
    let nonce = B64.decode(parts[0]).map_err(|_| SyncError::Other("b64".into()))?;
    let ct = B64.decode(parts[1]).map_err(|_| SyncError::Other("b64".into()))?;
    let nonce_arr: [u8; 12] = nonce.as_slice().try_into().map_err(|_| SyncError::Other("nonce len".into()))?;
    let plain = KeyStore::new(&KeyStore::derive_key().map_err(|e| SyncError::Other(e.to_string()))?)
